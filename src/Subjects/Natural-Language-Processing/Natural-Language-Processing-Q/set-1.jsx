...
const Quiz = () => {
    const questions = [
        ...
        {
          question: "What happens to audio data before being processed by ML models?",
          options: [
        ...
        {
          question: "Is there any true exception to numerical representation in Machine Learning?",
          options: [
  ...
  const generateReport = () => {
    ...
    responses.forEach((r, i) => {
      ...
      const ansLines = doc.splitTextToSize(ansText, pageWidth - 2 * margin - 20);
      doc.setFont("helvetica", "normal");
...
const generateCertificate = () => {
    ...
    doc.setFontSize(48);
    doc.text("Certificate of Completion", pageWidth / 2, margin + 60, { align: "center" });
  
    // 4. Subtitle
    doc.setFont("helvetica", "bold");
    doc.setFontSize(30);
    doc.setTextColor(50, 50, 90);
    doc.text("RNotion Challenge", pageWidth / 2, margin + 120, { align: "center" });
  
    // 5. Issuer line
    doc.setFont("helvetica", "normal");
    doc.setFontSize(16);
    doc.setTextColor(80, 80, 80);
    doc.text("by Rayyan's Notion", pageWidth / 2, margin + 150, { align: "center" });
  
    // 6. Horizontal separator
    doc.setDrawColor(100, 100, 100);
    doc.setLineWidth(0.5);
    doc.line(pageWidth/2 - 200, margin + 170, pageWidth/2 + 200, margin + 170);
    ...
    doc.text(`${participantName}`, pageWidth / 2, margin + 270, { align: "center" });
    ...
    doc.text('"Natural Language Processing"', pageWidth / 2, margin + 340, { align: "center" });
  
    const description = "Demonstrating outstanding knowledge, critical thinking, and commitment to excellence.";
    const descLines = doc.splitTextToSize(description, pageWidth - 2 * margin - 100);
    doc.setFont("times", "normal");
    doc.setFontSize(14);
    ...
    doc.setFontSize(14);
    doc.setTextColor(50, 50, 50);
    doc.text("Authorized Signature", pageWidth / 2, pageHeight - margin - 20, { align: "center" });
    ...
    <div className={styles.wrapper}>
      ...
      >
        {!finished ? (
          ...
          >
            <h2><sub><GraduationCap size={70}/></sub>&nbsp;&nbsp;<sup>RNotion Challenge</sup></h2>
            ...
            <p className={styles.feedback}>
              {pct === 100
                ...
                ? (
                    <>
                      Good Effort.<br />
                      <span style={{fontSize: "1rem", color: "red", marginLeft: "8px"}}>Scrore above 75% to get the certificate.</span>
                    </>
                  )
                : (
                  <>
                    Continuous practice is recommended.<br />
                    <span style={{fontSize: "1rem", color: "red", marginLeft: "8px"}}>Scrore above 75% to get the certificate.</span>
                  </>
                )}
            </p>
                ...
                {pct > 75 ? (
                  <button className={styles.pdfButton} onClick={() => {generateCertificate(); generateReport();}}>
        ...
        {finished && pct > 75 && (
            <div style={{ border: '5px solid black', borderRadius: '10px', position: 'absolute', top: '-30%', left: '-100%' }}>  
              <Confetti
                width={1536}
                height={776}
                recycle={false}
